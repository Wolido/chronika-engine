...
const TOOLS: Record<string, ToolHelp> = {
  world_gen: {
    ...
    parameters: [
      ...
      { name: "monsters", type: "array", required: false, description: "怪物数组，每个需: name/category(beast/mutant/humanoid/mechanical/abomination)/hp/damage_min/damage_max/accuracy(0-1)/evasion(0-1)/armor/tier(1-5)/xp_reward/strength(1-20)/agility(1-20)/endurance(1-20)/perception(1-20)/intelligence(1-20)/willpower(1-20)", constraints: "statin_sum ≤ tier×17, accuracy+evasion ≤ 1.3" },
      { name: "items", type: "array", required: false, description: "物品数组，每个需: name/item_type(consumable/material/armor/misc)/rarity(common/uncommon/rare/legendary)/value", constraints: "consumable需要effect_type/effect_value" },
      { name: "status_effects", type: "array", required: false, description: "状态效果数组" },
